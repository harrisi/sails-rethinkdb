...
const debug = require('debug')('sails-rethinkdb-query')
...
module.exports = class Query {
  ...
  static build(table, query) {
    debug('Query build', table, query)
    return new Query(table).build(query)
  }

  constructor(table) {
    debug('Query constructor', table)
    this.table = table
  }
  ...
  buildWhere(t, segment) {
    ...
    debug('buildWhere', segment.id, Object.keys(segment).length)
  ...
  buildExpression(row, field, segment) {
    debug('buildExpression', row, field, segment)
